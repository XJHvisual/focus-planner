...
class BuiltinTracker:
    ...
    def _get_foreground(self):
        ...
        try:
            ...
            if pid.value:
                kernel32 = ctypes.windll.kernel32
                h_process = kernel32.OpenProcess(0x0400 | 0x0010, False, pid.value)  # QUERY_INFO | VM_READ
                if h_process:
...
class TimeTrackTab(ttk.Frame):
    ...
    def __init__(self, parent, app):
        ...
        outer = ttk.Frame(self)
        outer.pack(fill="both", expand=True)
        cv = tk.Canvas(outer, bg="#FFFFFF", highlightthickness=0)
        ...
        def _on_cv_conf(e): cv.itemconfig("inner", width=e.width)
        def _on_inner_conf(e): cv.configure(scrollregion=cv.bbox("all"))
        cv.bind("<Configure>", _on_cv_conf)
        self.inner.bind("<Configure>", _on_inner_conf)
        cv.bind("<MouseWheel>", lambda e: cv.yview_scroll(int(-1 * e.delta / 120), "units"))

        # ── 概览卡片 ──
        cards_frame = ttk.Frame(self.inner)
        ...
        # ── 应用排行柱状图 ──
        bar_section = ttk.LabelFrame(self.inner, text="📋 应用排行（今日）", padding=10)
        bar_section.pack(fill="x", padx=12, pady=4)

        self.bar_canvas = tk.Canvas(bar_section, bg="#FAFAFA", height=180, highlightthickness=0)
        self.bar_canvas.pack(fill="x")

        # ── 分类分布 ──
        pie_section = ttk.LabelFrame(self.inner, text="🎨 分类分布（今日）", padding=10)
        pie_section.pack(fill="x", padx=12, pady=4)

        self.pie_canvas = tk.Canvas(pie_section, bg="#FAFAFA", height=160, highlightthickness=0)
        self.pie_canvas.pack(fill="x")

        # ── 状态栏 ──
        status_frame = ttk.Frame(self.inner)
    ...
    def refresh(self):
        ...
        try:
            ...
            # 今日总时长
            cur = conn.execute(
                "SELECT COALESCE(SUM(duration_seconds),0) FROM usage_records WHERE start_time LIKE ?",
                (today + "%",)
            )
            ...
            if total_sec == 0:
                conn.close()
                self._set_empty("📭 今日暂无记录")
                return

            # 高效时间（productivity_score >= 70）
            cur = conn.execute(
                "SELECT COALESCE(SUM(duration_seconds),0) FROM usage_records WHERE start_time LIKE ? AND productivity_score >= 70",
                (today + "%",)
            )
            ...
            # 最常用应用
            cur = conn.execute(
                "SELECT app_name, SUM(duration_seconds) as td FROM usage_records WHERE start_time LIKE ? GROUP BY app_name ORDER BY td DESC LIMIT 1",
                (today + "%",)
            )
            row = cur.fetchone()
            ...
            # 应用排行 Top 10
            cur = conn.execute(
                "SELECT app_name, SUM(duration_seconds) as td FROM usage_records WHERE start_time LIKE ? GROUP BY app_name ORDER BY td DESC LIMIT 10",
                (today + "%",)
            )
            apps = cur.fetchall()
            ...
            # 分类数据
            cur = conn.execute(
                "SELECT category, SUM(duration_seconds) as td FROM usage_records WHERE start_time LIKE ? GROUP BY category ORDER BY td DESC",
                (today + "%",)
            )
            cats = cur.fetchall()
            ...
            self.card_vars["cat_count"].set(str(len(cats)))

            # 绘制柱状图
            self._draw_bars(apps, total_sec)
            # 绘制分类条
            self._draw_category_bars(cats, total_sec)
    ...
    def _set_empty(self, msg):
        ...
        self.pie_canvas.create_text(300, 80, text=msg, font=("Microsoft YaHei", 10), fill="#999")
    ...
    def _draw_bars(self, apps, total_sec):
        ...
        bar_area_w = w - 150
        bar_h = max(12, (h - 40) // len(apps))
        gap = 6
        font = ("Microsoft YaHei", 10)

        for i, (name, td) in enumerate(apps):
            y = 20 + i * (bar_h + gap)
            bw = max(4, int((td / max_td) * bar_area_w)) if max_td > 0 else 4
            color = self.CARD_COLORS[i % len(self.CARD_COLORS)]

            # 标签
            short = (name[:18] + "…") if len(name) > 20 else name
            cv.create_text(75, y + bar_h // 2, text=short, font=font, anchor="e", fill="#333")
            # 条
            cv.create_rectangle(85, y, 85 + bw, y + bar_h, fill=color, outline="", tags="bar")
            # 时长
            m, s = divmod(td, 60)
            hh, m = divmod(m, 60)
            dur = f"{hh}h{m}m" if hh > 0 else f"{m}m{s}s"
            cv.create_text(92 + bw, y + bar_h // 2, text=f"{dur}", font=font, anchor="w", fill="#555")

    def _draw_category_bars(self, cats, total_sec):
        cv = self.pie_canvas
        cv.delete("all")
        w = cv.winfo_width() or 600
        h = 160
        if not cats or total_sec == 0:
            return
        ...
        for i, (cat, td) in enumerate(cats):
            y = start_y + i * (bar_h + gap)
            pct = (td / total_sec) * 100 if total_sec > 0 else 0

            cv.create_text(65, y + bar_h // 2, text=cat, font=font, anchor="e", fill="#333")

            # 背景条
            cv.create_rectangle(75, y, w - 80, y + bar_h, fill="#E8E8E8", outline="")
            # 前景条
            fw = int((pct / 100) * (w - 155)) if pct > 0 else 2
            color = self.CARD_COLORS[i % len(self.CARD_COLORS)]
            cv.create_rectangle(75, y, 75 + fw, y + bar_h, fill=color, outline="")
            # 百分比
            m, s = divmod(td, 60)
            hh, m = divmod(m, 60)
            dur = f"{hh}h{m}m" if hh > 0 else f"{m}m"
            cv.create_text(82 + fw, y + bar_h // 2, text=f"{pct:.0f}% ({dur})",
                          font=font, anchor="w", fill="#555")
